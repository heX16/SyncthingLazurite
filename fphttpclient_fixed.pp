...
Type
  ...
  public
    ...
    constructor CreateFmt(const Msg: string; const Args: array of const; AErrorCode: Integer = 0);
    property ErrorCode: Integer read FErrorCode;
  end;

  // Specialized HTTP Client Exceptions
  // Usage examples:
  //   Raise CreateSocketException('Socket error', seHostNotFound);
  //   Raise CreateSocketExceptionFromCode('Socket read error', -1);
  //   Raise EHTTPClientTimeout.Create('Connection timeout');
  EHTTPClientSocketError = Class(EHTTPClient);
  EHTTPClientTimeout = Class(EHTTPClient);
  EHTTPClientConnectionLost = Class(EHTTPClient);
  EHTTPClientHostNotFound = Class(EHTTPClient);
  EHTTPClientConnectFailed = Class(EHTTPClient);
  EHTTPClientConnectTimeout = Class(EHTTPClient);
...
// Helper function to create appropriate exception based on socket error type
function CreateSocketException(const Msg: string; ErrorType: TSocketErrorType): EHTTPClient;
begin
  case ErrorType of
    seHostNotFound:
      Result := EHTTPClientHostNotFound.Create(Msg, Ord(ErrorType));

    seCreationFailed:
      Result := EHTTPClientSocketError.Create(Msg, Ord(ErrorType));

    seConnectFailed:
      Result := EHTTPClientConnectFailed.Create(Msg, Ord(ErrorType));

    seConnectTimeOut:
      Result := EHTTPClientConnectTimeout.Create(Msg, Ord(ErrorType));

    seIOTimeOut:
      Result := EHTTPClientTimeout.Create(Msg, Ord(ErrorType));
...
// Helper function to create appropriate exception based on system error code from recv()
function CreateSocketExceptionFromCode(const Msg: string; ErrorCode: Integer): EHTTPClient;
begin
  case ErrorCode of
    // Timeout errors
    10060, // WSAETIMEDOUT on Windows
    110,   // ETIMEDOUT on Unix
    -1:    // EWOULDBLOCK/EAGAIN on Unix (often indicates timeout)
      Result := EHTTPClientTimeout.Create(Msg, ErrorCode);

    // Connection lost/closed errors
    10054, // WSAECONNRESET on Windows
    104:   // ECONNRESET on Unix
      Result := EHTTPClientConnectionLost.Create(Msg, ErrorCode);

    // Host not found errors
    11001, // WSAHOST_NOT_FOUND on Windows
    11002, // WSATRY_AGAIN on Windows
    11003, // WSANO_RECOVERY on Windows
    11004, // WSANO_DATA on Windows
    1:     // EPERM on Unix (often used for host not found)
      Result := EHTTPClientHostNotFound.Create(Msg, ErrorCode);

    // Connection failed errors
    10061, // WSAECONNREFUSED on Windows
    111:   // ECONNREFUSED on Unix
      Result := EHTTPClientConnectFailed.Create(Msg, ErrorCode);

    else
      // Generic socket error
      Result := EHTTPClientSocketError.Create(Msg, ErrorCode);
  end;
end;
...
begin
  ...
  try
    ...
    begin
      FreeAndNil(FSocket);
      Raise CreateSocketException(E.Message, E.Code); // E.Code is TSocketErrorType
    end;
    on E: Exception do
    begin
      FreeAndNil(FSocket);
...
function TFPCustomHTTPClient.ReadString(out S: String): Boolean;
  ...
  begin
    ...
    If (r=0) or Terminated Then
      Exit(False);
    If (r<0) then
      Raise CreateSocketExceptionFromCode(SErrReadingSocket, r);
...
Function TFPCustomHTTPClient.ReadResponse(Stream: TStream;
  ...
  begin
    ...
    If Result<0 then
      Raise CreateSocketExceptionFromCode(SErrReadingSocket, Result);
    if (Result>0) then
      begin
  ...
  var
    ...
    begin
      ...
      If Cnt<0 then
        Raise CreateSocketExceptionFromCode(SErrReadingSocket, Cnt);
...
constructor EHTTPClient.Create(const Msg: string; AErrorCode: Integer);
begin
  inherited Create(Msg);
  FErrorCode := AErrorCode;
end;

constructor EHTTPClient.CreateFmt(const Msg: string; const Args: array of const; AErrorCode: Integer);
begin
